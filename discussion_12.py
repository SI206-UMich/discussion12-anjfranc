...
def create_employee_table(cur, conn):
    cur.execute("create table if not exists employees [employee_id INTEGER PRIMARY KEY, first_name TEXT, last_name TEXT, job_id INTEGER, hire_data DATE, salary NUMBER]")
    conn.commit()
...
def job_and_hire_date(cur, conn):
    cur.execute("select jobs.job_title, employees.hire_date from jobs join employees on jobs.job_id= employyes.job_id order by employees.hire_date limit 1")
    x=cur.fetchoone()
    return x[0]
